...
RISC系CPU (LEON4 SPARC V8) 2-Core を搭載しており、RTOS (Real Time Operating System) のスケジューリング、パイプライン管理、センサー操作を担当している。  
...
ただ MA2485 は 2020Q2 に生産中止、現在は MA2085 のみが提供されている。  
...
CPU に Arm A53 4-Core を搭載し、これによりスタンドアローンで Linuxシステムを動作させることができるようになった。  
4-15W という範囲の電力で動作し、M.2カードや複数を搭載する PCIeカードのフォームファクタに対応する。  
推論性能において、NVIDIA Jetson TX2 と比較では、電力効率は 6.2倍、面積あたりの性能は 8.7倍優れているとされる。  
また、*Myriad X* との比較では 10倍の推論性能を持つとしている。  

ただ競合製品と比較したときの性能効率は公開されているが、VPU部や Neural Compute Engine、オンチップメモリ CMX の規模、*Myriad X* からの改良点は公開されていない。  
また Intel AI Summit 2019 では 2020年の前半に発売予定としていたが、具体的な SKU については未だ発表されていない。  
製造プロセスについても同様。  
...
*Myriad X* 1-Chip が 0.7 TOPS とされ、**3700VE** は 7.1 TOPS 持つため、ここで上記の推論性能 10倍が達成されている。  
...
専用ハードウェアは ECDH-256, ECDH-384 アルゴリズムに対応。  

 * [[PATCH 0/5] Keem Bay OCS ECC crypto driver](https://lore.kernel.org/linux-devicetree/20211020103538.360614-1-daniele.alessandrelli@linux.intel.com/T/#m0a328ead665886f48ae0c60a79d694662042468a)

## Thunder Bay {#thb}
まだ公式発表はされていないが、Intel Movidius VPU には *Thunder Bay (THB)* なる SoC も存在し、Linux Kernel にパッチが投稿されている。  
...
*Thunder Bay* は CPUクラスタ 4基を持ち、クラスタごとに *Keem Bay* と同じく Arm A53 4-Core を持つ。  
