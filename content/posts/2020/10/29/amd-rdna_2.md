...
*RDNA 2 アーキテクチャ* ではそこに *Infinity Cache / L3キャッシュ* 、メモリチャネルあたり 8MB が新設された、ということだろうか。  

上述した **Radeon RX 6000 Series** のダイショットでは、動画内で示されていた *Infinity Cache* とされる部分はそれ程大きく占めてはいないように見える。  
そのため、メモリを構成するトランジスタ数が少ない eDRAM 等の可能性があるが、確実ではない。AMD のさらなる発表が待たれる。  
